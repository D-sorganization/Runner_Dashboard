...
| 2026-09-26 | #1579 | Code-reviewer runtime fixes (#1579): the author-provider lookup works against the real `RunStore` (filters by repo; no `list_runs(repo=)`); selection uses the roster's `code-reviewer.providers` and `Agent-Id` trailers via `GhCliPrProbe.get_commit_messages`; same-provider reviews carry `SAME_PROVIDER_TAG` through to a `(same-provider)` outcome; auto-review submits via `Runner.submit` from plain threads, dedupes per PR and reports failures; non-numeric `pr` is `invalid_params`. |
