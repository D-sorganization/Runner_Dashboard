...
| 2026-09-25 | #1322 | SC-E5: Stalled-job detection and remediation playbooks for the Maintenance role: 5 diagnostic detectors (queued_too_long, running_past_p95, wedged_listener, offline_with_job, ghost_runners) with fault isolation; autonomous self-heal for low-risk issues (restart listener, cancel+rerun stale queue, clean worktrees) capped at 3/host/hour; repeat failures escalate through Barb; medium/high risk issues create ActionProposal records awaiting operator approval; endpoints POST /api/v1/staff/maintenance/scan and POST /api/v1/staff/maintenance/remediate. |
...
- **2026-09-25:** Stalled-Job Detection and Remediation Playbooks for Maintenance (issue #1322, SC-E5). Implemented diagnostic detectors and automated remediation playbooks for the Maintenance role (`backend/staff/maintenance_detect.py`, `backend/staff/maintenance_playbooks.py`, `backend/routers/staff_maintenance.py`). Added 5 diagnostic detectors with fault isolation: `detect_queued_too_long` (queued > 30m with matching idle runners), `detect_running_past_p95` (running jobs exceeding historical p95 × 3 or 120m fallback), `detect_wedged_listener` (online/busy runners whose local listener log mtime has not updated for > 300s), `detect_offline_with_job` (runners offline in GitHub with assigned unresolved jobs), and `detect_ghost_runners` (registrations missing from local fleet inventory or offline > 7 days). Remediator adheres to owner decision policy: only low-risk actions (restart wedged listener, cancel + rerun stale queued job, clean orphaned worktrees) execute autonomously, with a thread-safe rate limit of 3 self-heal actions per host per hour; repeat failures on the same target within 1 hour escalate through Barb as medium-risk action proposals; medium and high-risk issues generate `ActionProposalRecord` items and `WorkItemStore` entries awaiting operator approval. Self-heal execution, escalation, and proposal creation post system notifications to the dedicated Maintenance conversation thread and record audit entries in `staff_audit`. Mounted REST endpoints under `/api/v1/staff`: `POST /api/v1/staff/maintenance/scan` (scoped `staff.read`) and `POST /api/v1/staff/maintenance/remediate` (scoped `fleet.maintain`). All modules strictly <= 500 lines. TDD: `tests/unit/test_staff_maintenance_detect.py` (13 tests) and `tests/api/test_staff_maintenance_api.py` (6 tests).
