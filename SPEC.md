...
- **2026-06-15 (2.5.146):** Continued #949 frontend monolith retirement by
  ...
  now owns `/api/repos`, `/api/feature-requests`, prompt-template loading, and
  feature-request dispatch/save calls outside the legacy App owner while
  preserving the prop-driven `FeatureRequestsTab` contract for fallback callers,
  and `RoutedShell` bypasses the legacy chunk for `/t/feature-requests`.
