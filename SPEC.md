...
| ---------- | ---------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-09-25 | #1489 | SC-B1-G6: Secrets are redacted at every conversation and run persistence boundary, not just message bodies. One structured redactor (`redaction.redact_value`, shape-preserving over dicts/lists/tuples) covers thread titles and meta, message meta, proposal params and decision/transition reasons, run `prompt`/`target_ref`/`error`/`last_line`/`remediation`, run event text and every transcript line written by the runner. |
| 2026-09-25 | #1485 | SC-B1-G2: Harden the action-proposal API. Creating a proposal needs `staff.chat`, a registered action and a real thread plus a message in it; the risk always comes from the action registry (`registered_risk`, unknown = high), never from the caller, a chat reply, a group turn or the assistant model. `execute_proposal` runs only `approved` proposals (`approve=True` is the one-step approve-and-execute, recorded after every policy check); `/execute` on anything else is 409, and a `failed` proposal needs an explicit retry decision. The action's `required_scope` is enforced at execute; results are never posted to a missing thread. |
| 2026-09-25 | #1487                  | SC-B1-G4: one staff-dispatch policy. `staff/dispatch_service.dispatch_staff_run(DispatchCommand, principal)` owns consolidation, plan validation, the `dispatches` rate limit, machine targeting (`local`/peer/`auto`), peer forwarding with signed on-behalf-of provenance, dry-run and the fail-closed `dispatch` audit; `POST /api/staff/{role}/run` and the `staff.dispatch`/`staff.review_pr` actions both call it, so an approved proposal with `machine=<peer>` now forwards, is rate limited, honours `ctx.dry_run` and is audited with `surface=thread`. Executor refusals are classified (`invalid_params`, `rate_limited`, `peer_unreachable`, `upstream_error`, `permission_denied`, `bridge_unavailable`); the verifier accepts forwarded runs and dry runs. The worker-thread → loop bridge is lifted to `staff/loop_bridge.run_on_loop` and shared with `maintenance_github.call_github`. |
| 2026-09-25 | #1285                  | CR-4: Planner stage backend. A strong-tier planner (profile role `planner`) is dispatched through the CR-3 envelope with the target repo's AGENTS.md/CLAUDE.md and a JSON output contract; its plan (posted to `POST /api/code-requests/{id}/plan` or as a `<!-- plan:v1 -->` issue comment ingested by `.../plan/ingest`) is validated, not trusted: required child sections, `tier:*`/`complexity:*`/Conductor task-class labels, acyclic dependencies, and a dashboard-rendered turnover document that must pass the fleet handoff rules (vendored from RM `handoff_validator.py`, drift-tested). Invalid plans are re-prompted with the errors (2 retries) then the request moves `planning → failed`. With `plan_requires_approval` (default) nothing is written to GitHub until `.../plan/approve`, which files the epic and children in wave order, posts `<!-- turnover:v1 -->` comments, links sub-issues (resumable, no duplicates) and moves the request to `planned` with `plan_epic`. |
| 2026-09-25 | #1491 | SC-B1-G8: Reconcile chat messages stuck in pending/streaming after a backend restart. On startup, mark every non-terminal reply message as failed with `meta.failure_class="interrupted_by_restart"`, post a system retry message to the thread, audit every state change under SC-A8, and leave user messages and complete messages untouched. |
| 2026-09-25 | #1502                  | Fix Fleet Orchestration false successes: dispatch returns classified `upstream_error` and `dispatched: false` when `gh` fails, injects `machine_target` into workflow dispatch inputs, and deploy returns 501 `not_wired` while still recording the audit attempt. Added API test suite in `tests/api/test_orchestration_dispatch.py`. |
| 2026-09-25 | #1522                  | Restore green main: align generated API contract types (`frontend/src/lib/api-types.ts`) via `scripts/gen-api-client.sh`, removing formatting and trailing newline drift introduced after #1512 to ensure `generate-api:check` passes cleanly in CI. |
| 2026-09-25 | #1493                  | SC-B1-G10: Stream chat tokens live instead of after the process exits. Implemented `LiveProcessReader` and `stream_turn_output` in `backend/staff/chat_streaming.py` to incrementally consume stdout lines via background thread, publishing token deltas to ThreadEventBus as they arrive while CLI process is running, recording accurate time-to-first-token (TTFT), and terminating orphaned subprocesses on cancellation. Added unit tests in `tests/unit/test_staff_chat_streaming.py`. |
| 2026-09-25 | #1479                  | K2: knowledge packs in the Staff Console — vendored knowledge engine from Tools (SHA 09ff428), pack refresh systemd service/timer and backend module, retrieval-augmented chat turns injecting cited ## Knowledge block for roles with search_knowledge, read-only Pydantic-typed knowledge endpoints (GET /api/v1/staff/knowledge/{pack_id} and /search), and advisors roster group with routing rules for disciple and vision-quest.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| 2026-09-25 | #1513                  | Restore green main: align OpenAPI schema disambiguation (`proposals__models__CreateProposalRequest`) with Python 3.11 contract in CI, ensuring `Frontend Tests` (`generate-api:check`) passes completely.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| 2026-09-25 | #1513                  | Restore green main: synchronize generated OpenAPI schema and TypeScript definitions (`openapi.json`, `api-types.ts`) following SC-B9 group threads endpoint addition (`/api/v1/staff/groups/{group_id}/threads`), ensuring `generate-api:check` passes in CI.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| 2026-09-25 | #1492                  | SC-B1-G9: Chat pool saturation rejects turns with chat_capacity instead of executing anyway. Implemented `ChatConcurrencyPool.acquire(role, timeout)` respecting total slot capacity and Barb reservation, configured `DEFAULT_CHAT_ACQUIRE_TIMEOUT = 5.0` seconds, added `record_chat_capacity_failure()` in `backend/staff/chat_failures.py` updating placeholder messages to failed with failure class `chat_capacity`, and updated `ChatTurnRunner.execute_turn` to acquire a slot before dispatching, immediately returning retryable `chat_capacity` failure on saturation without attempting LLM execution or skewing successful turn metrics. Added dedicated unit test suite in `tests/unit/test_staff_chat_capacity.py`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| 2026-09-25 | #1448                  | SC-E3 wiring slice 1: `maintenance.run_cancel`, `maintenance.run_rerun` and `maintenance.cancel_and_rerun` call GitHub for real through `staff.maintenance_github` (org-scoped repos only; cancel verified by conclusion `cancelled`, rerun by an advanced `run_attempt`; cancel_and_rerun waits for the cancel to settle). Staff executors and the stalled-job scan now run in a worker thread (`anyio.to_thread`) and bridge to the loop-bound `gh_client`; new failure classes `upstream_error` and `bridge_unavailable`. Adds `gh_client.rerun`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
| 2026-09-25 | #1339                  | SC-B9: Group threads: talk to the Board (and other groups) with the Board-Secretary coordinating seat replies. Added group data models (`group_models.py`) and group coordination engine (`groups.py`) supporting Board definition (seats Alpha, Bravo, Charlie, Delta + coordinator `board-secretary`); concurrent fanout across seat chat turns with per-seat timeout and error resilience; consensus collation with quorum threshold, markdown synthesis, expandable seat disclosures, and automatic `board.propose` ActionProposal generation; pre-send token/USD cost estimation and threshold guard (`STAFF_GROUP_COST_THRESHOLD_USD`, default $2.00) requiring explicit confirmation (`confirm_cost=True`); endpoints `GET /api/v1/staff/groups`, `GET /api/v1/staff/groups/{id}`, `GET /api/v1/staff/groups/{id}/cost-estimate`, `POST /api/v1/staff/groups/{id}/threads`; integration into `POST /api/v1/staff/threads` and `POST /api/v1/staff/threads/{id}/messages` with background async coordinator runner; authorized `board-secretary` for `board.propose` and `staff.dispatch` actions; fixed `create_work_item` parameter naming in `action_executors.py`. All files strictly <= 500 lines. Comprehensive TDD: `tests/unit/test_staff_groups.py` and `tests/api/test_staff_groups_api.py`.                                                                                                |
| 2026-09-25 | #1474                  | WP-0.1: Resolve staff action role names against the loaded roster. Replaced unresolvable role literals in `backend/staff/action_executors.py` with module constants: `DEFAULT_REVIEWER_ROLE = "fleet-critic"`, `CODE_REQUEST_OWNER_ROLE = "barb"`, `BOARD_PROPOSAL_ROLE = "board-secretary"`. Updated `execute_review_pr` to default to `DEFAULT_REVIEWER_ROLE`, `execute_code_request_create` to default to `CODE_REQUEST_OWNER_ROLE`, and `execute_board_propose` to default to `BOARD_PROPOSAL_ROLE`. Added `validate_action_default_roles` checking `load_roles()`, logging warnings without crashing the server at runtime and raising on error in tests. Added unit test suite in `tests/staff/routing_eval/test_action_executor_roles.py`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| 2026-09-25 | #1287                  | CR-5: Executor stage — route planned issues to cheaper agents with claims, escalation and rollup. Implemented dependency wave execution, executor resolution (pinned profile vs Conductor cheapest capable in-budget provider), multi-agent coordination (claim check, lease posting, PR metadata validation `Fixes #N` and `agent:<agent>`), durable turnover handoff propagation, tier escalation (ollama -> cli -> strong) on 2 failures, human triage pause (`needs-human-triage`) upon strong tier exhaustion, repository concurrency limits, and aggregate rollup with per-child state and cost tracking (`backend/code_requests/executor_stage.py`, `executor_models.py`, `executor_router.py`, `executor_coordination.py`). Mounted REST endpoints in `backend/routers/code_requests_executor.py` and `backend/server.py` (`POST /api/code-requests/{id}/executor/initialize`, `POST /api/code-requests/{id}/executor/dispatch`, `POST /api/code-requests/{id}/executor/report-child`, `GET /api/code-requests/{id}/executor/rollup`). Synchronized OpenAPI schema and TypeScript bindings. Added comprehensive unit and route integration test suites.                                                                                                                                                                                                                                             |
