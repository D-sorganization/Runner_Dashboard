...
| 2026-09-26 | #1587                  | Live subscription quota: `GET /api/staff/quota` reports Claude windows (stream `rate_limit_event`, status-line script) and Codex windows (session logs) per provider; `/api/usage` shows them as `<provider>_plan` percent sources and the invented April fixture is emptied. |
...
| 2026-09-26 | #1488 | SC-B1-G5: Relay forwarded run-card events back to originating thread across peer nodes (Option A). Propagated `origin_node` and `thread_id` across `RunRequest`, `RunPlan`, `RunRecord`, `RunBody`, `DispatchCommand` and `runner.submit`. When executing on a remote peer, `handle_run_status_change` posts run card updates to `POST /api/v1/staff/threads/{thread_id}/relay-card` on the originating node. Origination endpoint verifies target thread existence (404), updates card idempotently in place without duplicate messages, and preserves terminal execution status (`completed`, `failed`, `cancelled`) against delayed out-of-order intermediate card deliveries. Relay failure mid-run logs visibly on the executing node without crashing the execution path. |
...
| 2026-09-26 | #1579 | WP-1.3 follow-up: the code-reviewer finds the PR author in the run store, selects from the role's providers with an `Agent-Id` trailer fallback, records `(same-provider)` in the stored outcome, and auto-review queues through `runner.submit` once per PR. A non-numeric `pr` is `invalid_params`. |
| 2026-09-26 | #1518 | WP-1.3: Code-reviewer runtime. `DEFAULT_REVIEWER_ROLE = "code-reviewer"` with fallback to `fleet-critic`. Provider selection picks first available provider in `code-reviewer.providers` differing from the PR author's provider family (falling back to commit trailers `Agent-Id`); if only one provider family is available, selects alternate model and flags `same-provider`. Parses `STAFF_RESULT: review approve|changes|escalate #<pr>` into run outcome; missing verdict line becomes `needs_input`. Advisory mode enforces comment reviews only and never passes request-changes. Optional `STAFF_AUTO_REVIEW=0/1` auto-dispatches review when a run is verified on a P0/P1 repo. |
| 2026-09-25 | #1517 | WP-1.2: agent outcome scorecard. `GET /api/v1/staff/outcomes?since=&group_by=role|provider|repo` (default: 14 days, by role) reports runs, verified rate, PRs, merge rate, CI first-pass rate (first commit's check runs), fix-within-48h rate (another PR merged for the same issue within 48 h) and cost per merged PR. A rate with no data is `null`, never 0; PRs GitHub could not read count as `prs_unknown`; at most 50 PRs are read per request (newest first, cached; `prs_truncated` says so). Shown as the Staff **Outcomes** tab. |
| 2026-09-26 | #1503 | SC-G5-6: Retire legacy dispatch forms and endpoints. Retired QuickDispatch (popover, schemas, CSS, and route) and Jules remediation dispatch (helper, Run button, and route). `POST /api/agents/quick-dispatch` and `POST /api/agent-remediation/dispatch-jules` return HTTP 410 Gone with Link (`/api/v1/staff/requests`) and Sunset headers. Old client paths redirect to Staff Console (`/`). Updated frontend integrity tests. |
| 2026-09-25 | #1346 | SC-G8: delete dead frontend code (never-mounted primitives, dead hooks, orphaned legacy pages). Removed legacy-only `QuickDispatch.tsx` and `AlertsCenter.tsx`, the unreferenced `alertAck.ts` helper and `schemas/dispatch.ts` schema, their tests, the `AlertsCenter` barrel export, and orphaned `.quick-dispatch` styles. Added static integrity test guarding against re-introduction of dead frontend code. Bundle JS size is unchanged at 1,014,235 B (already tree-shaken); index CSS reduced by 1.27 kB. |
...
| 2026-09-26 | #1562 | Wire the new `mad-scientist` staff role (Repository*Management#1788): Barb deterministic routing keywords (mad scientist, frankenstein, wild idea, lab bench, lab experiment, lab:promote) with routing-eval cases lab-01..04, and the role joins the Advisors group in the Staff Console roster. |
...
| POST   | `/api/agent-remediation/dispatch-jules` | Retired (HTTP 410 Gone); successor `/api/v1/staff/requests` (SC-G5-6 #1503) |
...
| POST   | `/api/agents/quick-dispatch` | Retired (HTTP 410 Gone); successor `/api/v1/staff/requests` (SC-G5-6 #1503)           |
