...
| 2026-09-24 | #1296 | SC-A10: Contract check between backend response models and frontend types for staff and conversation APIs; strongly typed Pydantic models for all staff routes; frontend types derived from generated OpenAPI schema; CI contract drift check. |
...
- **2026-09-24:** Staff API OpenAPI Contract & Response Typing (issue #1296, SC-A10). Implemented strongly typed Pydantic models for all staff responses in `backend/routers/staff_models.py` (`StaffRoleSpec`, `StaffRosterResponse`, `StaffRunRecord`, `StaffBoardResponse`, `StaffSummaryResponse`, `StaffRunsResponse`, `StaffRunDetailResponse`, `StaffCancelResponse`, `StaffDispatchResponse`, `StaffAuditResponse`, `StaffHoldsResponse`, `StaffScheduleResponse`, `StaffUsageResponse`, etc.). Applied `response_model` across `backend/routers/staff.py`, `backend/routers/staff_schedule.py`, and `backend/routers/staff_usage.py`. Mounted backwards-compatible `/api/v1/staff/*` router in `backend/routers/staff_v1.py`. Updated `frontend/src/pages/Staff/staffApi.ts` to derive all TypeScript data shapes directly from generated OpenAPI schemas (`frontend/src/lib/api-types.ts`). Added contract check tests in `tests/frontend/test_api_generation_contract.py` asserting all staff routes define explicit 200/201 JSON response models and verifying deliberate drift detection.
