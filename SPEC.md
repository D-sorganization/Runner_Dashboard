...
- **2026-09-24:** Structured Reply Contract for Chat Turns (issue #1308, SC-B5). Implemented conversational reply parser in `backend/staff/reply_contract.py` supporting plain markdown prose, optional fenced ```staff-actions JSON block (`[{action, params, reason}]`), optional `handoff: <role>`, and optional `question: <text>`. Implemented strict JSON schema validation and action vocabulary check across standard and maintenance actions. Enforced role permission checks against declared permissions, fleet_actions, and tools/scopes, dropping unauthorized or unknown actions with system notes. Adversarial protection ignores prompt-injected action blocks and directives inside markdown blockquotes (`>`) and nested code fences. Fail-soft degradation guarantees parser exceptions are impossible by construction, preserving the prose reply with warning reporting. Extended `backend/staff/roles.py` `RoleSpec` with `defers_to` property and flexible persona parsing. Updated `backend/staff/workspace.py` with `contract_text()` loading and `chat_mode` prompt composition with `CHAT_RULES`. TDD: `tests/unit/test_reply_contract.py` (19 tests covering schema, degradation, permissions, adversarial blockquotes, and all 13 RM chat role playbook fixtures).
