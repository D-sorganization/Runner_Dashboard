...
| 2026-09-25 | #1551                  | Staff chat failure card keeps the most specific classified failure across the fallback chain with its remediation: `select_best_failure` ranks specific failure classes above generic provider errors and tie-breaks by earliest provider; `execute_turn` records the winning candidate's failure class, remediation, and error; `MessageRecord` surfaces `remediation` in `meta` and dictionary serialization. |
