...
Handles queue operations including:
  - Workflow queue endpoints (/api/queue)
  - Job dispatch and management
  - Queue status and metrics
  - Queue diagnostics and bottleneck identification
  - Individual run cancellation and rerun
"""
...
from utils.utilities import gh_api_admin, run_cmd
...
# Cache for queue and diagnostic data
# Note: This is a module-local cache; the main server.py cache is passed in via set_cache_store()
_cache: OrderedDict[str, tuple[Any, float]] = OrderedDict()
MAX_CACHE_SIZE = 500
_CACHE_EVICT_BATCH = 50

# Cache wrapper functions (delegated to utilities)
def _cache_get(key: str, ttl: float) -> Any | None:
    """Return cached value if within TTL, else None."""
    from utils.utilities import _cache_get as util_cache_get
    return util_cache_get(key, ttl, _cache)


def _cache_set(key: str, data: Any, _ttl: float | None = None) -> None:
    """Store value with current timestamp."""
    from utils.utilities import _cache_set as util_cache_set
    util_cache_set(key, data, _cache, MAX_CACHE_SIZE, _CACHE_EVICT_BATCH, _ttl)


# External cache store injection: allows server.py to pass its cache object
_external_cache: OrderedDict[str, tuple[Any, float]] | None = None


def set_cache_store(cache: OrderedDict[str, tuple[Any, float]]) -> None:
    """Set the external cache store (called from server.py after initialization).

    This allows the queue router to use the same cache instance as the main server,
    ensuring cache coherence across all operations.
    """
    global _external_cache
    _external_cache = cache


def _get_cache_store() -> OrderedDict[str, tuple[Any, float]]:
    """Get the active cache store (external if set, otherwise local)."""
    return _external_cache if _external_cache is not None else _cache


def _cache_get_shared(key: str, ttl: float) -> Any | None:
    """Return cached value from the active store."""
    from utils.utilities import _cache_get as util_cache_get
    return util_cache_get(key, ttl, _get_cache_store())


def _cache_set_shared(key: str, data: Any, _ttl: float | None = None) -> None:
    """Store value in the active store."""
    from utils.utilities import _cache_set as util_cache_set
    util_cache_set(key, data, _get_cache_store(), MAX_CACHE_SIZE, _CACHE_EVICT_BATCH, _ttl)
...
async def _queue_impl() -> dict:
    ...
    Queries the 15 most recently updated repos concurrently for both 'queued'
    and 'in_progress' statuses and aggregates the results.
    """
    cached = _cache_get_shared("queue", 120.0)
    ...
    _cache_set_shared("queue", result)
    return result


# Note: GET /api/queue endpoint is handled by server.py to support hub proxying.
# The _queue_impl() function is exported for use by server.py and other internal functions.


# ─── Run Control ──────────────────────────────────────────────────────────────


@router.post("/runs/{repo}/cancel/{run_id}")
async def cancel_run(
    ...
    *,
    ...
    cache.pop("queue", None)
    cache.pop("diagnose", None)
    return {"cancelled": True, "run_id": run_id, "repo": repo}


@router.post("/runs/{repo}/rerun/{run_id}")
...
@router.post("/queue/cancel-workflow")
async def cancel_workflow_runs(
    ...
    *,
    ...
    runs_to_cancel = [
        ...
        and (target_repo is None or (r.get("repository") or {}).get("name") == target_repo)
    ]
...
async def diagnose_queue() -> dict:
    ...
    """
    cached = _cache_get_shared("diagnose", 120.0)
    if cached is not None:
        return cached

    # Runner pool status
    try:
        runner_data = _cache_get_shared("runners", 25.0)
        if runner_data is None:
            runner_data = await gh_api_admin(f"/orgs/{ORG}/actions/runners")
            _cache_set_shared("runners", runner_data)
    ...
    pick_runner_misconfig = [
        ...
        if (j.get("job") or "").lower() in ("pick-runner", "pick runner", "select runner")
    ...
    if pick_runner_misconfig:
        ...
        bottleneck = (
            ...
            "The pick-runner job must use 'runs-on: ubuntu-latest' (not 'self-hosted') — "
            "it is the dispatcher that decides where to send work. "
            "Update those workflow files to fix 'runs-on: ubuntu-latest' on the pick-runner job."
        )
    elif waiting_for_fleet > 0 and not idle:
        bottleneck = (
            f"All {len(busy)} d-sorg-fleet runner(s) are busy. "
            "Jobs will run as runners finish. Bring more machines online to increase throughput."
        )
    elif waiting_for_fleet > 0 and idle:
        bottleneck = (
            f"{len(idle)} idle fleet runner(s) exist but {waiting_for_fleet} fleet job(s) are "
            "still queued — possible label mismatch. Verify the runner labels include 'd-sorg-fleet'."
        )
    elif waiting_for_generic_sh > 0 and idle:
        if runner_groups_restricted:
            blocked_info = [
                f"'{g['name']}' (runners: {', '.join(g['runner_names'][:3])}{'…' if len(g['runner_names']) > 3 else ''}) "
                f"blocks: {', '.join(g['blocked_waiting_repos'][:5])}"
                for g in runner_groups_info
                if g["restricted"] and g["blocked_waiting_repos"]
            ]
            bottleneck = (
                f"RUNNER GROUP ACCESS RESTRICTION: {waiting_for_generic_sh} job(s) cannot "
                f"reach {len(idle)} idle runner(s). "
                + (" | ".join(blocked_info) + ". " if blocked_info else "")
                + "FIX: GitHub org Settings → Actions → Runner Groups → "
                "select the restricted group → set Repository access to 'All repositories'."
            )
        else:
            bottleneck = (
                f"{waiting_for_generic_sh} job(s) target the generic 'self-hosted' label "
                f"with {len(idle)} idle runner(s). "
                "Runners will pick these up — but check if any are pick-runner "
        ...
        bottleneck = (
            f"All {len(busy)} fleet runner(s) are busy and {waiting_for_generic_sh} job(s) "
    ...
    _cache_set_shared("diagnose", result)
    return result
