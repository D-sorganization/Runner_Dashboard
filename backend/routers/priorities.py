...
Routes (all under ``/api/priorities``):
  ...
  GET /directives          Active (unexpired) operator directives.
  PUT /directives          Replace the directive list.

Auth: reads use ``require_fleet_peer``; the PUT uses ``coordination.auth.require_coordination_writer``
(principal with ``coordination.write`` or a loopback orchestrator peer) plus
the CSRF header enforced by middleware. Logic lives in ``priorities.service``.
"""
...
from priorities.directives import MAX_PRIORITY, MAX_SET_BY, MAX_TEXT, MIN_PRIORITY, get_directives
...
class DirectiveBody(BaseModel):
    ...
    set_by: str | None = Field(default=None, max_length=MAX_SET_BY)
...
async def get_directives_route(_peer: str = Depends(require_fleet_peer)) -> dict[str, Any]:
    ...
    return {"directives": [d.to_dict() for d in directives.active()], "path": str(directives.path)}


@router.put("/directives")
async def put_directives(
    body: DirectivesBody,
    caller: Caller = Depends(require_coordination_writer),  # noqa: B008
) -> dict[str, Any]:
    """Replace the directive list. ``set_by`` defaults to the authenticated caller. Returns the active list."""
    directives = get_directives()
    items = [{**d.model_dump(exclude_none=True), "set_by": d.set_by or caller.label} for d in body.directives]
    try:
        directives.replace(items)
