...
3. **Data Loss Warning**
   ...
   **Never DROP columns or tables** during a migration. If you must remove a column, first deploy a version that stops reading/writing it. Then, in the _next_ release, deploy the `ALTER TABLE ... DROP COLUMN` migration.
