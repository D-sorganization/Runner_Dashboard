...
- DeskComputer switched 2026-09-23 14:05 PT: owner turned off "Expose Ollama to the network" (restart needed before it took effect), disabled both `ollama.exe` inbound rules, ran `-Action Install` elevated. Ollama now listens on 127.0.0.1 only; forward `172.21.32.1:11434 -> 127.0.0.1:11434`; rule local 172.21.32.1 / remote 172.21.32.0/20 on `vEthernet (WSL (Hyper-V firewall))`. The first task runs backed up portproxy on every no-op run → backup now only when a forward changes.
