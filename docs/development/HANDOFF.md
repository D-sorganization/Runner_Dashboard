...
- Directives: `priorities.write` scope (operator only) via `priorities/auth.py`; `set_by` from the caller; one-line text; `version` + 409; invalid stored entries skipped; no `path` in responses.
...
- Not touched: `backend/coordination/*` (concurrent #1245). `priorities/auth.py` duplicates the shape of `coordination.auth.require_coordination_writer`; fold both into a scope-parameterised factory once #1245 lands.
- Validation: WSL `pytest tests/api -k "priorities or staff or auth" tests/clients` (only the known flaky `test_submit_runs_fake_cli_to_success_with_events_and_cost` intermittently fails on /mnt/c timing); `npx vitest run FleetCommand`, `npm run typecheck`, `npm run lint`, `npm run build` clean; OpenAPI snapshot regenerated.
