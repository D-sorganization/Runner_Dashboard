# Current handoff — Load full role definitions and surface invalid roles (#1300)
...
- `backend/staff/schema.json`:
  - Bundled normalized staff schema for standalone and test environments.
- `backend/staff/rm_sync.py`:
...
- `frontend/src/pages/Staff/Roster.tsx`:
  - Updated `RoleCard` to render `"invalid"` state badge with danger tone and show error message when a role file has schema errors.
- `tests/unit/test_staff_roles.py`:
  - Added 7 unit tests covering schema field parsing, valid role validation, schema error surfacing, corrupt YAML surfacing, mtime-keyed caching, validation error mapping in `source_status`, and runner dispatch rejection on invalid roles.
...
- `pytest tests/unit/test_staff_roles.py`: 7 passed.
