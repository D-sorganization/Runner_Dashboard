...
- **Started:** 2026-09-25
- **Last verified:** 2026-09-25 (review-defect fix pass: pytest 29/29 passing on `tests/unit/test_proposals_store.py` + `tests/api/test_proposals_routes.py` + `tests/clients/test_fleet_client_proposals.py`, plus targeted regression on `test_gh_client.py`/`test_gh_utils.py`/`test_no_subprocess_gh.py`/`test_routers_runners.py`/fleet client-cli-mcp suites — one pre-existing local-venv gap unrelated to this change, `ModuleNotFoundError: jwt` in `test_gh_client.py`'s GitHub App token tests, not introduced here; vitest 52/52 passing across `ProposalsPanel`/`CodeRequests`/`CodeRequestsPage`/`FleetCommand`/`FleetCommandHelpers`/`FleetCommandWrites` suites; `npx tsc -p tsconfig.app.json --noEmit` 0 errors; `ruff check`/`ruff format --check` clean; `py -3.12 -m mypy` clean; `bash scripts/gen-api-client.sh --check` clean; all touched files <= 500 lines)
