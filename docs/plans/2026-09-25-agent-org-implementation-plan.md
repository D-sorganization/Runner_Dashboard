...
- **Date:** 2026-09-25
- **Status:** proposed; the owner has not decided. Peer review with the active Runner_Dashboard sessions is in progress (see §6).
...
- **Evidence baseline:** Runner_Dashboard `7f73b08`

## 1. Why This Repository Carries Most of the Work

Staff roles are _defined_ in Repository_Management (`staff/roles/*.yml`) and _executed_ here. Four of the five gaps in the analysis are therefore Runner_Dashboard gaps:
...
| Vision-to-code pipeline incomplete          | `backend/code_requests/` (CR-4 to CR-8 unbuilt; approval flags unenforced)   |
| No outcome metrics                          | Next to `backend/routers/staff_usage.py` (cost only today)                   |

The role-file changes (new `code-reviewer`, `product-owner`, `chief-architect`) land in Repository_Management. Everything that makes those roles _run and be checked_ lands here.
...
- **Change:** implement the `board_proposals` source in `backend/staff/inbox.py`.
  - Read open Repository_Management issues labelled `board:proposal` through the existing `gh_client` REST helpers: one call, cached with the inbox TTL.
  - Map each issue to an `InboxItem` with kind `board_proposal`, its age, and a link.
...
| WP-2.3 Board gate (CR-6)                  | #1286          | `board-secretary`                                                                         | Requests flagged `board_route` wait in `board_review`                                                           |
| WP-2.4 Proposals API/tab (CR-7)           | #1284          | `board-secretary`                                                                         | Replaces the WP-0.2 GitHub read with the native store                                                           |
...
| ----------------------------------- | ---------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `claude-deskcomputer-20260925`      | #1340: `backend/staff/routing_eval.py`, `frontend/src/pages/Staff/Board.tsx` | Adjacent to WP-0.1 (routing). WP-0.1 touches `action_executors.py`, not `routing_eval.py`, but should land after #1340 or be coordinated with that session. |
| `claude-oglaptop-20260925-tracking` | #1345: `frontend/src/shell`                                                  | None.                                                                                                                                                       |
...
Sent through the fleet mailbox (RM#1576) on 2026-09-25 to `claude-deskcomputer-20260925` and `claude-oglaptop-20260925-tracking`. Replies will be folded in here and the status line updated.
