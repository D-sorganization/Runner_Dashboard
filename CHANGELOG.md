...
- Priorities and fleet clients hardening (#1243): bots can no longer set operator directives (new `priorities.write` scope, `set_by` always the authenticated caller); a checklist directive no longer 500s every staff dispatch; directive saves are optimistic-concurrency checked (409 + reload prompt in Fleet Command) and one bad stored entry no longer empties the list; `GET /api/priorities/directives` no longer returns the server path; `/api/prioritiesX` is no longer perimeter-exempt; `fleetctl claim`/`release-claim` without intent/reason no longer 422; `send-message --to '*'` works; MCP gains `fleet_ack_message`; clients derive `<agent>-<host>-<date>` sessions and reject sessions without the agent prefix; the Messages panel registers operator presence before sending; one scope-parameterised write dependency (`require_writer`); a coordination board refresh leaked from before a cache reset can no longer repopulate the cache (flaky stale-while-revalidate test now joins the refresh).
...
- Loopback-only Ollama for staff nodes (#1257): `deploy/ollama-wsl-bridge.ps1` gives WSL a narrow, reboot-safe bridge (portproxy + WSL-subnet-only firewall rule + re-apply task) to a Windows Ollama that listens on 127.0.0.1 only, replacing "Expose Ollama to the network".
...
- Staff Hub usage ledger: per-run `cost_method`, price table + wall-time fallback, `GET /api/staff/usage` grouped by provider/role/day with daily budget, and `POST /api/staff/usage/export` appending to Repository_Management `data/credit_usage.json` via subprocess (#1200, epic #1192).
- Staff tab: `frontend/src/pages/Staff/` with Board, Roster, Runs (live SSE tail + cancel), Assign (dry-run preview + dispatch) and Holds panels over `/api/staff/*`, registered as nav entry `staff` in the AI & Agents group (#1198, epic #1192).
- Staff Hub scheduler: cron schedules and run windows from role YAML, a persisted holds list (`GET/PUT /api/staff/holds`), per-role daily budgets with 75/90/100 % alerts, and `GET /api/staff/schedule` (#1196, epic #1192).
