...
import { MaxwellTab, type MaxwellStatus } from "../MaxwellPage";
...
function stubMaxwellFetch(opts: { tasks?: unknown[]; contract?: string; chatReply?: string } = {}): void {
  vi.stubGlobal(
    "fetch",
    vi.fn((url: string) => {
...
describe("MaxwellTab", () => {
  it("renders without throwing (smoke test)", () => {
    stubMaxwellFetch();
    expect(() =>
      render(<MaxwellTab status={STOPPED} loading={false} onControl={() => Promise.resolve()} />),
    ).not.toThrow();
  });

  it("renders the status stat row and contract version", async () => {
    stubMaxwellFetch({ contract: "v1.2.3" });
    render(<MaxwellTab status={RUNNING} loading={false} onControl={() => Promise.resolve()} />);
  ...
  it("shows Start when stopped and dispatches start", () => {
    ...
    render(<MaxwellTab status={STOPPED} loading={false} onControl={onControl} />);
    fireEvent.click(screen.getByRole("button", { name: "Start Maxwell daemon" }));
  ...
  it("dispatches stop when running", () => {
    ...
    render(<MaxwellTab status={RUNNING} loading={false} onControl={onControl} />);
    expect(screen.getByRole("button", { name: "Restart Maxwell daemon" })).toBeInTheDocument();
    fireEvent.click(screen.getByRole("button", { name: "Stop Maxwell daemon" }));
  ...
  it("dispatches restart when running", () => {
    ...
    const onControl = vi.fn(() => new Promise<void>(() => {}));
    render(<MaxwellTab status={RUNNING} loading={false} onControl={onControl} />);
    fireEvent.click(screen.getByRole("button", { name: "Restart Maxwell daemon" }));
    expect(onControl).toHaveBeenCalledWith({ action: "restart" });
  });

  it("reports a control failure to the operator", async () => {
    stubMaxwellFetch();
    const onControl = vi.fn(() => Promise.reject(new Error("systemctl denied")));
    render(<MaxwellTab status={STOPPED} loading={false} onControl={onControl} />);
    fireEvent.click(screen.getByRole("button", { name: "Start Maxwell daemon" }));
    await waitFor(() => expect(screen.getByText(/systemctl denied/)).toBeInTheDocument());
  });
  ...
  it("reports an unreachable daemon when the chat request fails", async () => {
    vi.stubGlobal(
      "fetch",
      vi.fn((url: string) => {
        if (String(url).includes("/api/maxwell/chat")) return Promise.reject(new Error("boom"));
        return Promise.resolve(jsonResponse(String(url).includes("version") ? { contract: "" } : { tasks: [] }));
      }),
    );
    render(<MaxwellTab status={RUNNING} loading={false} onControl={() => Promise.resolve()} />);
    ...
    await waitFor(() =>
      expect(
        screen.getByText("Maxwell-Daemon is unreachable. Check daemon status above, then retry."),
      ).toBeInTheDocument(),
    );
  });
  ...
  it("Refresh invokes onRefresh", () => {
    ...
    render(
      <MaxwellTab status={RUNNING} loading={false} onRefresh={onRefresh} onControl={() => Promise.resolve()} />,
    );
    fireEvent.click(screen.getByRole("button", { name: "Refresh Maxwell status" }));
    expect(onRefresh).toHaveBeenCalledTimes(1);
  });

  it("renders fetched recent tasks", async () => {
    stubMaxwellFetch({ tasks: [{ id: "task-one-zzzz", status: "done", repo: "x/y", created_at: "2026-06-02T10:00:00Z" }] });
    render(<MaxwellTab status={RUNNING} loading={false} onControl={() => Promise.resolve()} />);
  ...
  it("shows offline task message when daemon unreachable", async () => {
    ...
    // fetch resolves.
    await waitFor(() =>
      expect(screen.getByText("Maxwell-Daemon offline — no task history")).toBeInTheDocument(),
    );
  });
  ...
  it("renders an error banner", () => {
    stubMaxwellFetch();
    render(
      <MaxwellTab status={STOPPED} loading={false} error="daemon exploded" onControl={() => Promise.resolve()} />,
    );
  ...
  it("sends a chat message and renders the reply", async () => {
    ...
    expect(screen.getByText("status?")).toBeInTheDocument();
    await waitFor(() => expect(screen.getByText("fleet is healthy")).toBeInTheDocument());
  });
  ...
  it("offers quick-action chips that send canned prompts", async () => {
    ...
    await waitFor(() => expect(screen.getAllByText("ok").length).toBeGreaterThanOrEqual(1));
  });
  ...
  it("surfaces a Retry control and unreachable hint when offline", () => {
    ...
    expect(
      screen.getByPlaceholderText("Daemon unreachable; retry before sending commands"),
    ).toBeInTheDocument();
  });

  it("Retry re-fetches and invokes onRefresh when offline", async () => {
    const fetchSpy = vi.fn((url: string) =>
      Promise.resolve(jsonResponse(String(url).includes("version") ? { contract: "" } : { tasks: [] })),
    );
    vi.stubGlobal("fetch", fetchSpy);
    const onRefresh = vi.fn();
    render(
      <MaxwellTab status={STOPPED} loading={false} onRefresh={onRefresh} onControl={() => Promise.resolve()} />,
    );
  ...
  it("reveals a Latest scroll-to-bottom control when scrolled up", async () => {
    ...
    const list = document.querySelector(".maxwell-chat-messages") as HTMLElement;
    ...
    await waitFor(() => expect(screen.getByRole("button", { name: "Scroll to bottom of chat" })).toBeInTheDocument());
    fireEvent.click(screen.getByRole("button", { name: "Scroll to bottom of chat" }));
  });
});
