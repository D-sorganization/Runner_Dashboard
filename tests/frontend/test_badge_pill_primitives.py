...
def test_ask_sheet_consumes_touch_button() -> None:
    """Issue #1499: AskSheet replaces AgentDispatch and consumes TouchButton."""
    source = _read(SRC_DIR / "shell" / "AskSheet.tsx")
    assert 'from "../primitives/TouchButton"' in source
    assert "<TouchButton" in source
    assert 'className="btn' not in source
...
def test_push_settings_consumes_empty_state_and_touch_button() -> None:
    """Issue #834 migration: PushSettings should use shared primitives."""
    source = _read(SRC_DIR / "pages" / "PushSettings.tsx")
    assert 'from "../primitives/EmptyState"' in source
    assert 'from "../primitives/TouchButton"' in source
    assert "<EmptyState" in source
    assert "<TouchButton" in source
    assert "push-settings__topic" in source
    assert 'className="btn' not in source
    assert "style={{" not in source
