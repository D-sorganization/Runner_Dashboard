...
def _check_mermaid_views(text: str) -> tuple[int, int]:
    ...
    if not context_matches:
        raise ArchitectureMapContractError(
            "Architecture map missing required 'C4Context' view block in "
            "mermaid format."
        )
    ...
    if not container_matches:
        raise ArchitectureMapContractError(
            "Architecture map missing required 'C4Container' view block in "
            "mermaid format."
        )
...
def _parse_feature_map(text: str) -> list[dict[str, str]]:
    ...
    for line in text.splitlines():
        ...
        if in_feature_map:
            ...
            if (
                ...
                if len(parts) >= MIN_FEATURE_COLUMNS:
    ...
    if not feature_entries:
        raise ArchitectureMapContractError(
            "Feature Map table is empty or missing required columns "
    ...
    for line in text.splitlines():
        ...
        if in_changelog:
            ...
            if (
                ...
                parts = [p.strip() for p in line.strip("|").split("|")]
                if len(parts) >= MIN_CHANGELOG_COLUMNS:
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        sys.stdout.write(
            ...
            f"{res.container_views} container, "
            f"{len(res.feature_map_entries)} features, "
